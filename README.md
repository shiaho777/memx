...
- Coordination plane: `REGS <name>` registers a runtime client (peer pid via `LOCAL_PEERPID`; record cleared on disconnect), `BEAT <compressed> <resident> <saved>` pushes stats, `POLI` returns the current system pressure level (0/1/2 from the daemon's `DISPATCH_SOURCE_TYPE_MEMORYPRESSURE` subscription, decaying after 60 s quiet), `CLNT` lists clients, `PRES <n>` forces a level for tests when `MEMX_STORE_DEBUG=1`. `STAT` reports `clients=`/`level=`.
- Runtime attach: `MEMX_SERVICE=1` makes `libmemx_runtime` spawn a service thread — `REGS` (program name, `MEMX_SERVICE_NAME` override), `BEAT` + `POLI` every `MEMX_SERVICE_BEAT_MS` (default 5000). Level ≥1 → soft trim `trim(32)`; level ≥2 → `trim(512|32)`. Daemon absent → quiet reconnect backoff; the client never touches the fault path. `MEMX_SERVICE_SOCK` overrides the socket path.
