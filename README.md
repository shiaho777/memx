...
Intentional changes use **Issue → PR → CI → merge**. Issues close only when the fix is on `main`.
...
1. Open or reuse a GitHub Issue: https://github.com/shiaho777/memx/issues
...
4. Put `Fixes #N` or `Closes #N` in the PR body. Do not close the Issue when the PR opens or while CI is red.
5. Required check: **CI / gate** ([`.github/workflows/ci.yml`](.github/workflows/ci.yml), job id `gate`). Do not merge red. CI does not close Issues; merge does.
...
FullHost and vessel demos need local model weights under `MEMX_MODEL_PATH` (not committed).
...
Repo admins should require job **`gate`** from workflow **CI** on `main`. Docs alone cannot enforce GitHub branch protection.
...
1. **Base branch is always `main`.** Feature PRs into `main` only unless a maintainer names another base.
2. **Issue first** for intentional code/doc/process changes. Reuse an open Issue when one already tracks the work.
3. **Close Issue on merge only.** PR body includes `Fixes #N` or `Closes #N`. Never close when the PR opens, while CI is red, or before merge.
4. **CI is the merge gate.** Required check: workflow **CI**, job **`gate`**. Wait for green; fix and push on red. Do not merge red. CI must not auto-close Issues.
5. **One primary Issue per PR** when possible.
...
9. **User overrides** win for that turn only. State the override in the PR/Issue comment or handoff.

Verify: `make all` && `make test`. Optional when touching Python bindings: `make test-python-bitexact`.
