...
_Static_assert(MEMX_ROLE_ACTIVATION == MEMX_TENSOR_ROLE_ACTIVATION, "role");
_Static_assert(MEMX_ROLE_EMBEDDING == MEMX_TENSOR_ROLE_EMBEDDING, "role");
_Static_assert(MEMX_ROLE_TEMPORARY == MEMX_TENSOR_ROLE_TEMPORARY, "role");
_Static_assert(MEMX_ROLE_DATA == MEMX_TENSOR_ROLE_DATA, "role");
_Static_assert(MEMX_DTYPE_UNKNOWN == MEMX_TENSOR_DTYPE_UNKNOWN, "dtype");
_Static_assert(MEMX_DTYPE_FP16 == MEMX_TENSOR_DTYPE_FP16, "dtype");
_Static_assert(MEMX_DTYPE_BF16 == MEMX_TENSOR_DTYPE_BF16, "dtype");
_Static_assert(MEMX_DTYPE_FP32 == MEMX_TENSOR_DTYPE_FP32, "dtype");
_Static_assert(MEMX_DTYPE_INT8 == MEMX_TENSOR_DTYPE_INT8, "dtype");
_Static_assert(MEMX_DTYPE_UINT8 == MEMX_TENSOR_DTYPE_UINT8, "dtype");
_Static_assert(MEMX_DTYPE_INT32 == MEMX_TENSOR_DTYPE_INT32, "dtype");
_Static_assert(MEMX_FLAG_READ_MOSTLY == MEMX_TENSOR_FLAG_READ_MOSTLY, "flag");
_Static_assert(MEMX_FLAG_SEQUENTIAL == MEMX_TENSOR_FLAG_SEQUENTIAL, "flag");
_Static_assert(MEMX_FLAG_HOT == MEMX_TENSOR_FLAG_HOT, "flag");
_Static_assert(MEMX_FLAG_NO_COMPRESS == MEMX_TENSOR_FLAG_NO_COMPRESS, "flag");
_Static_assert(MEMX_FLAG_COLD == MEMX_TENSOR_FLAG_COLD, "flag");
_Static_assert(MEMX_CODEC_TENSOR_FP16_SPLIT == MEMX_RUNTIME_CODEC_TENSOR_FP16_SPLIT, "codec");
_Static_assert(MEMX_CODEC_TENSOR_BITPLANE16 == MEMX_RUNTIME_CODEC_TENSOR_BITPLANE16, "codec");
_Static_assert(MEMX_CODEC_TENSOR_SPARSE_BYTE == MEMX_RUNTIME_CODEC_TENSOR_SPARSE_BYTE, "codec");
_Static_assert(MEMX_CODEC_TENSOR_FP16_DELTA_SPLIT == MEMX_RUNTIME_CODEC_TENSOR_FP16_DELTA_SPLIT, "codec");
_Static_assert(MEMX_CODEC_ZLIB == MEMX_RUNTIME_CODEC_ZLIB, "codec");
_Static_assert(MEMX_CODEC_TENSOR_FP16_ZLIB_SPLIT == MEMX_RUNTIME_CODEC_TENSOR_FP16_ZLIB_SPLIT, "codec");
_Static_assert(MEMX_CODEC_TENSOR_EXP_PACK == MEMX_RUNTIME_CODEC_TENSOR_EXP_PACK, "codec");
...
static int memx_core_hook_inflate(const uint8_t *src, uint32_t src_len, uint8_t *dst, uint32_t *dst_len) {
    uLongf dl = *dst_len;
    if (memx_inflate_once(src, src_len, dst, &dl) != Z_OK) return -1;
    *dst_len = (uint32_t)dl;
    return 0;
}

static int memx_core_hook_uncompress(const uint8_t *src, uint32_t src_len, uint8_t *dst, uint32_t expected_len) {
    uLongf dl = expected_len;
    if (uncompress(dst, &dl, src, src_len) != Z_OK || dl != expected_len) return -1;
    return 0;
}

__attribute__((constructor)) static void memx_core_hooks_install(void) {
    static memx_core_hooks_t memx_core_hooks_storage;
    memx_core_hooks_storage.inflate = memx_core_hook_inflate;
    memx_core_hooks_storage.uncompress = memx_core_hook_uncompress;
    memx_core_set_hooks(&memx_core_hooks_storage);
}
...
static void encode_tensor_page_one(MemXZone3 *s, size_t pidx, const uint8_t *src, uint8_t *tensor_dst,
    ...
    int try_sparse = tensor_sparse_byte_eligible(&s->meta[pidx]);
    int try_fp16 = tensor_fp16_split_eligible(&s->meta[pidx]);
    ...
    while(s->running){
        ...
        for(size_t i=0;i<nc;) {
            ...
            for(size_t k=i; k<j; k++) {
                ...
                __sync_synchronize();
                if (!page_compress_content_ok(&s->meta[pidx], seq0, dstp, (const uint8_t*)s->vmem+pidx*PAGE_SZ)) {
        ...
        for(size_t i=0;i<nc;i++){
            ...
            if(is_zero) {
                ...
                page_csz[i] = 8;
            } else if (tensor_sparse_byte_eligible(&s->meta[tc[i]]) || tensor_fp16_split_eligible(&s->meta[tc[i]]) ||
            ...
            for(int probe=0; probe<8; probe++) {
                ...
                if (s->dedup_hash[s2] == h && s->dedup_sz[s2] == cs && s->dedup_ref[s2] > 0) {
                    ...
                    if (pool_blob_eq_locked(s, existing_off, cdata, cs)) {
                        ...
                        if (!page_compress_content_ok(&s->meta[pidx], page_seq[i], s->tmp_src + i * PAGE_SZ, (const uint8_t*)s->vmem+pidx*PAGE_SZ)) {
                            if (s->meta[pidx].state == PAGE_COMPRESSING) {
                                s->meta[pidx].state = PAGE_HOT;
                                s->meta[pidx].cooldown = 6;
                                hot_list_add(s, (uint32_t)pidx);
...
static int force_compress_page_now(MemXZone3 *s, size_t pidx) {
    ...
    if (!page_compress_content_ok(&s->meta[pidx], seq0, src, (const uint8_t*)s->vmem+pidx*PAGE_SZ)) {
        restore_compressing_page(s, pidx);
        return 0;
    }
    pthread_mutex_lock(&s->alloc_mutex);
    ...
    for (int probe = 0; probe < 24; probe++) {
        ...
        if (s->dedup_hash[s2] == h && s->dedup_sz[s2] == csz && s->dedup_ref[s2] > 0) {
            ...
            if (pool_blob_eq_locked(s, existing_off, dst, csz)) {
                if (!page_compress_content_ok(&s->meta[pidx], seq0, src, (const uint8_t*)s->vmem+pidx*PAGE_SZ)) break;
                __sync_fetch_and_add(&s->dedup_ref[s2], 1);
                m->pool_offset = existing_off;
                m->codec = codec;
                m->comp_size = csz;
                __sync_synchronize();
                if (!page_compress_content_ok(&s->meta[pidx], seq0, src, (const uint8_t*)s->vmem+pidx*PAGE_SZ) || m->dirty || m->write_seq != seq0) {
                    m->pool_offset = 0;
                    m->codec = 0;
                    m->comp_size = 0;
                    __sync_fetch_and_sub(&s->dedup_ref[s2], 1);
